...
const confirmUser = async (req: Request, res: Response) => {
	try {
      ...
      .redirect(`http://${process.env.ADDRESSPORT}/login`);
	} catch (ex) {
...
const confirmUser = async (req: Request, res: Response) => {
  try {
    const userInfo = (await jwt.verify(req.params.token, config.get('jwtPrivateKey'))) as User & mongoose.Document<any>

    const user = await userModel.findByIdAndUpdate(userInfo._id, { isVerified: true }, { new: true })
    if (!user) return res.status(StatusCodes.NOT_FOUND).send('User not found')

    res.status(StatusCodes.OK).redirect(`http://${process.env.ADDRESSPORT}/login`)
  } catch (ex) {
    res.status(StatusCodes.BAD_REQUEST).send('Invalid token.')
  }
}
